...
describe("MetaTokenTransfer", function () {
  it("Should let user transfer tokens thorough a relayer", async () => {
    ...
    // Get three address, trea ones as the user address
    // one as the relayer address, and one as a recipient adress
    const [_, userAddress, relayerAddress, recipientAddress] =
      await ethers.getSigners();

    // Mint 10000 tokens to user address (for testing)
    const tenThousandTokensWithDecimal = parseEther("10000");
    ...
    const mintTxn = await userTokenContractInstance.freeMint(
      tenThousandTokensWithDecimal
    );
    await mintTxn.wait(1);

    // Have the user infitnitea approve the token sender contract for transferring 'RandomToken'
